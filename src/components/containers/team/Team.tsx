  ...
  return (
    <section className="p-8 space-y-8" ref={ref}>
      ...
      >
        ...
        <p className="md:w-3/4 mx-auto leading-6 [&:not(:first-child)]:mt-6 font-mont">
      ...
      <div
        className={cn(
          "flex gap-y-10 gap-x-4 flex-wrap justify-center items-center",
          isInView && "animate-fade-up animate-delay-500",
        )}
      >
        {teamConfig.team.map((item) => {
          return (
            <div key={item.name} className="text-center space-y-4">
              <img
                src={item.img}
                alt={item.name}
                className="h-64 w-56 object-cover hover:scale-105 hover:shadow-md rounded-lg  transition-all overflow-hidden mb-4"
              />
              <div>
